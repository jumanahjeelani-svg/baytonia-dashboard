    ...
    <div className="flex flex-col flex-1 overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        <div className="grid grid-cols-4 gap-4">
          <MetricCard
            label="المتابعون"
            value={tiktokOverview.followers.value}
            change={tiktokOverview.followers.change}
            icon={Users}
            iconBg="bg-indigo-50"
          ...
          <MetricCard
            ...
            value={tiktokOverview.totalViews.value}
            change={tiktokOverview.totalViews.change}
            icon={Eye}
            iconBg="bg-violet-50"
          ...
          <MetricCard
            ...
            value={tiktokOverview.totalLikes.value}
            change={tiktokOverview.totalLikes.change}
            icon={Heart}
            iconBg="bg-rose-50"
            iconColor="text-rose-600"
            description="على جميع الفيديوهات"
          />
          <div className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm hover:shadow-md transition-shadow">
            <div className="flex items-start justify-between mb-4">
              ...
              <span className="flex items-center gap-1 text-xs font-semibold px-2 py-1 rounded-full bg-emerald-50 text-emerald-600">
                <TrendingUp size={11} />
                {tiktokOverview.engagementRate.change}%
              </span>
            </div>
            <p className="text-slate-500 text-sm mb-1">معدل التفاعل</p>
            <p className="text-slate-900 text-2xl font-bold">
              {tiktokOverview.engagementRate.value}
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            {
              icon: Play, iconBg: "bg-indigo-50", iconColor: "text-indigo-600",
              label: "مشاهدات الملف الشخصي", value: fmt(tiktokProfile.profileViews),
              sub: `+${tiktokProfile.profileViewsChange}% هذا الشهر`,
            },
            {
              icon: Clock, iconBg: "bg-emerald-50", iconColor: "text-emerald-600",
              label: "متوسط وقت المشاهدة", value: `${tiktokProfile.avgWatchTime}ث`,
              sub: "لكل فيديو",
            },
            {
              icon: TrendingUp, iconBg: "bg-rose-50", iconColor: "text-rose-600",
              label: "معدل إكمال المشاهدة", value: `${tiktokProfile.completionRate}%`,
              sub: "من المشاهدين يُكملون الفيديو",
            },
