...
import { dailyStoryViews, weeklyEngagementByDay, audienceByAge } from "@/lib/snapchat-data";
import { useState } from "react";

type MetricKey = "views" | "screenshots" | "replies" | "swipeUps";

const metrics: { key: MetricKey; label: string; color: string }[] = [
  { key: "views",       label: "مشاهدات",  color: "#FFAA00" },
  { key: "screenshots", label: "لقطات",    color: "#FF6B35" },
  { key: "replies",     label: "ردود",     color: "#6366F1" },
  { key: "swipeUps",    label: "تمرير لأعلى", color: "#10B981" },
];
...
export function StoryViewsChart() {
  ...
  return (
    <div className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm">
      <div className="flex items-start justify-between mb-4">
        <div>
          <h2 className="text-slate-800 font-bold text-base">أداء القصص اليومي</h2>
          ...
          {metrics.map((mt) => (
            <button
              ...
              className={`px-2.5 py-1 rounded-lg text-xs font-medium transition-colors ${
                active === mt.key ? "text-white" : "text-slate-500 hover:bg-slate-50"
              }`}
      ...
      <ResponsiveContainer width="100%" height={240}>
        <AreaChart data={dailyStoryViews} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
          <defs>
            <linearGradient id={`snap-${active}`} x1="0" y1="0" x2="0" y2="1">
...
export function WeeklyChart() {
  const highDays = ["الخميس", "الجمعة", "السبت"];
  return (
    <div className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm">
      <div className="mb-4">
        <h2 className="text-slate-800 font-bold text-base">أفضل أيام النشر</h2>
        <p className="text-slate-400 text-xs mt-0.5">متوسط مشاهدات القصص يومياً</p>
      </div>
        ...
        <BarChart data={weeklyEngagementByDay} margin={{ top: 4, right: 4, left: -20, bottom: 0 }} barSize={22}>
          ...
          <Bar dataKey="views" radius={[6, 6, 0, 0]}>
            {weeklyEngagementByDay.map((entry, i) => (
              <Cell
                key={i}
                fill={highDays.includes(entry.day) ? "#FFAA00" : "#FEF3C7"}
              />
            ))}
          </Bar>
        </BarChart>
      ...
      <p className="text-xs text-slate-400 text-center mt-2">
        <span className="text-amber-500 font-semibold">الخميس–السبت</span> أعلى أداءً للقصص
      </p>
    </div>
  );
}
  ...
  return (
    <div className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm">
      <div className="mb-4">
        ...
        <p className="text-slate-400 text-xs mt-0.5">توزيع المشتركين</p>
      </div>
            ...
            <div className="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden">
              <div
                ...
                style={{
                  ...
                  background: a.percent === maxPct ? "#FFAA00" : "#FDE68A",
                }}
              />
            </div>
            <span className={`text-xs font-bold w-8 text-left shrink-0 ${a.percent === maxPct ? "text-amber-600" : "text-slate-500"}`}>
              {a.percent}%
            </span>
          </div>
        ))}
      ...
      <p className="text-xs text-slate-400 mt-4 border-t border-slate-100 pt-3">
        الشريحة الأكبر: <span className="text-amber-600 font-semibold">25–34 سنة</span> (38%)
      </p>
    </div>
  );
}
